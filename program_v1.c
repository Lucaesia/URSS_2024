...
event integration (i++)
{
  ...
  a[right] = a[] + Delta*(s-1)*s_t*((BETA/(M*ALPHA))+a[])/D_a;

  foreach() {
    c[] = c[] + dt*(c[1] + c[-1] -2*c[])/sq(Delta*s) + dt*x*(c[1]-c[-1])*(s_t)/(2*Delta*s);
    a[] = a[] + dt*D_a*(a[1] + a[-1] -2*a[])/sq(Delta*(1-s)) + dt*x*(a[1]-a[-1])*(s_t)/(2*Delta*(1-s));
