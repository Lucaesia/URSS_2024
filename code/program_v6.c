...
// value to stop simulation when something is below the concentraion
double concentration_tolerance = 0.01;
...
event end_concentration_test (i+=1){
  ...
  if (a_left <= concentration_tolerance){
    printf("%g 1\n",t);
    return 1;
  }
  if (c_left <= concentration_tolerance){
    printf("%g 0\n",t);
    return 1;
  }
}
...
event logstats (t += 0.1, t<t_end) {
    double a_left = interpolate(a, EPS);
    printf("%g %g\n",t,a_left);
